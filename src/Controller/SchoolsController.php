...
class SchoolsController extends AppController
{
    ...
    public function demoSchool()
    {
        ...
        if ($systemSchool) {
            ...
            $systemAccountCount = $this->Accounts->find()
                ->contain(['Users'])
                ->where(['Users.school_id' => $systemSchool->id])
                ->count();

            $systemStats = [
                'count' => $systemAccountCount,
                'school' => $systemSchool
            ];
        }

        $this->set(compact('demoSchools', 'demoStats', 'systemStats'));
    }
    ...
    /**
     * Create system accounts for sample transactions
     * Creates 10 fixed "external" companies that all schools can transfer to
     *
     * @return \Cake\Http\Response|null
     */
    public function createSystemAccounts()
    {
        $this->request->allowMethod(['post']);

        # Prüfen ob System-Schule bereits existiert
        $systemSchool = $this->Schools->find()
            ->where(['kurzname' => 'system'])
            ->first();

        if ($systemSchool) {
            $this->Flash->error(__('System-Konten existieren bereits.'));
            return $this->redirect(['action' => 'demoSchool']);
        }
        ...
        # System-Schule erstellen
        $systemSchool = $this->Schools->newEntity([
            'name' => 'EduBank System',
            'kurzname' => 'system',
            'bic' => 'EDUBANKSYS',
            'ibanprefix' => 'SY00',
            'status' => 'system',
            'contact_email' => 'system@edubank.at'
        ]);

        if (!$this->Schools->save($systemSchool)) {
            $this->Flash->error(__('System-Schule konnte nicht erstellt werden.'));
            return $this->redirect(['action' => 'demoSchool']);
        }

        # 10 System-Konten (fiktive Geschäftspartner)
        $systemFirms = [
            ['name' => 'Bürobedarf Mustermann GmbH', 'desc' => 'Büromaterial, Schreibwaren'],
            ['name' => 'Druckerei Gutenberg OG', 'desc' => 'Druckaufträge, Visitenkarten'],
            ['name' => 'IT-Service Fischer KEG', 'desc' => 'EDV-Support, Hardware'],
            ['name' => 'Reinigung Sauber & Co', 'desc' => 'Reinigungsservice'],
            ['name' => 'Catering Lecker GmbH', 'desc' => 'Verpflegung, Events'],
            ['name' => 'Versicherung Sicher AG', 'desc' => 'Betriebsversicherung'],
            ['name' => 'Werbung Kreativ OG', 'desc' => 'Marketing, Werbemittel'],
            ['name' => 'Möbel Modern GmbH', 'desc' => 'Büroeinrichtung'],
            ['name' => 'Elektro Blitz KEG', 'desc' => 'Elektroinstallation'],
            ['name' => 'Transport Schnell GmbH', 'desc' => 'Lieferungen, Logistik'],
        ];

        $created = 0;
        foreach ($systemFirms as $index => $firm) {
            # User erstellen (nicht einloggbar)
            $username = 'system-' . ($index + 1);
            $user = $this->Users->newEntity([
                'username' => $username,
                'password' => bin2hex(random_bytes(16)), # Zufälliges Passwort (nicht nutzbar)
                'name' => $firm['name'],
                'role' => 'user',
                'school_id' => $systemSchool->id,
                'active' => 0  # Nicht einloggbar
            ]);
        ...
        $this->Flash->success(__('System-Konten erstellt: {0} Geschäftspartner verfügbar.', $created));
        return $this->redirect(['action' => 'demoSchool']);
    }
     ...
     * @return \Cake\Http\Response|null
     */
    public function deleteSystemAccounts()
    {
        $this->request->allowMethod(['post']);
        ...
        if (!$systemSchool) {
            $this->Flash->error(__('Keine System-Konten vorhanden.'));
            return $this->redirect(['action' => 'demoSchool']);
        }
        ...
        # Alle Transaktionen mit System-Konten löschen
        ...
        # User und Konten löschen
        $this->Accounts->deleteAll(['user_id IN' => $this->Users->find()->select(['id'])->where(['school_id' => $systemSchool->id])]);
        $this->Users->deleteAll(['school_id' => $systemSchool->id]);
        $this->Schools->delete($systemSchool);

        $this->Flash->success(__('System-Konten wurden gelöscht.'));
        return $this->redirect(['action' => 'demoSchool']);
    }
