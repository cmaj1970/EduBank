    ...
    <div class="col-lg-8 col-md-10">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <!-- Hidden TAN field for form submission -->
                <?= $this->Form->hidden('tan', ['id' => 'tan']) ?>
...
/* Modal über ALLES legen (auch sticky navbar) */
#mobileConfirmModal {
...
<div class="modal fade" id="mobileConfirmModal" tabindex="-1" data-bs-backdrop="false" aria-labelledby="mobileConfirmModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="margin: 0; max-width: 100%; width: 100%; height: 100%;">
        <div class="modal-content border-0 d-flex align-items-center justify-content-center" style="background: transparent; height: 100%;">
            ...
            <div class="modal-backdrop-custom" id="modalBackdropClose"></div>
                                ...
                                <div id="stateInitial">
                                    ...
                                    <div class="border rounded-2 p-2 mb-2" style="background: #f8f9fa; font-size: 0.75rem;">
                                        ...
                                        <div class="mb-1">
                                            ...
                                            <span id="modalIban" class="font-monospace" style="font-size: 0.65rem;">-</span>
                                        </div>
                                        <div class="mb-1">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <strong id="modalAmount" class="text-primary" style="font-size: 1rem;">-</strong>
                                        </div>
                                    </div>

                                    <button id="btnConfirm" class="btn btn-success w-100 py-1" style="font-size: 0.8rem;">
                                        <i class="bi bi-check-lg me-1"></i>Bestätigen
                                    </button>

                                    <button id="btnCancel" class="btn btn-secondary w-100 py-1 mt-2" style="font-size: 0.8rem;">
                                        <i class="bi bi-x-lg me-1"></i>Abbrechen
                                    </button>
                                </div>
...
$(document).ready(function() {
    ...
    function resetModalStates() {
        $('#stateInitial').show();
        $('#stateProcessing, #stateSuccess').hide();
        $('#modalTan').val('').removeClass('is-invalid');
        $('#tanError').hide();
    }
    ...
    $('#modalBackdropClose').on('click', function() {
        bootstrap.Modal.getInstance($('#mobileConfirmModal')[0]).hide();
    });

    // TAN-Request Button - öffnet Modal
    $('#requesttan').on('click touchend', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Formulardaten ins Modal übertragen
                $('#modalRecipient').text($('#empfaenger-name').val());
                $('#modalIban').text(formatIBAN($('#empfaenger-iban').val()));
                $('#modalPurpose').text($('#zahlungszweck').val());
                $('#modalAmount').text($('#betrag').val() + ' €');

                // Datum formatieren
                var dateVal = $('#datum').val();
                if (dateVal) {
                    var parts = dateVal.split('-');
                    $('#modalDate').text(parts[2] + '.' + parts[1] + '.' + parts[0]);
                } else {
                    $('#modalDate').text('Sofort');
                }

                // Progress und Modal öffnen
                updateProgressSteps(2);
                $('#formactions').hide();
                $('#transactionform').find(':input').prop('readonly', true);

                var modal = new bootstrap.Modal($('#mobileConfirmModal')[0]);
                modal.show();

                // Fokus auf TAN-Feld nach kurzer Verzögerung
                setTimeout(function() {
                    $('#modalTan').focus();
                }, 500);
            },
    ...
    // Bestätigen Button im Modal - PushTAN-Simulation (keine TAN-Eingabe nötig)
    $('#btnConfirm').on('click touchend', function(e) {
        e.preventDefault();

        // Processing State zeigen
        $('#stateInitial').hide();
        $('#stateProcessing').show();

        // Nach kurzer Verzögerung: Success zeigen und Formular absenden
        setTimeout(function() {
            $('#stateProcessing').hide();
            $('#stateSuccess').show();

            // Gültige TAN automatisch setzen (durch 7 teilbar)
            $('#tan').val('14007');
            ...
            // Progress aktualisieren und Formular absenden
